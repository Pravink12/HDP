...
heart_disease_model = pickle.load(open('C:/Users/Vikas Tarange/OneDrive/Desktop/mini project priyanshu/mini project/heart_disease_model.sav','rb'))
...
if (selected == 'Heart Disease Prediction'):
    ...
    with col2:
        sex = st.number_input('Male(1)/Female(0)')
        
    with col3:
        cp = st.number_input('Chest Pain types if yes type 1 else 0')
        
    with col1:
        trestbps = st.number_input('Resting Blood Pressure (Normaly : 120-130)')
        
    with col2:
        chol = st.number_input('Serum Cholestoral in mg/dl (Normaly : 200)')
        
    with col3:
        fbs = st.number_input('Fasting Blood Sugar > 120 mg/dl')
        
    with col1:
        restecg = st.number_input('Resting Electrocardiographic results(1,0)')
        
    with col2:
        thalach = st.number_input('Maximum Heart Rate achieved (Normaly : 60-100)')
        
    with col3:
        exang = st.number_input('Exercise Induced Angina(1,0)')
        
    with col1:
        oldpeak = st.number_input('ST depression induced by exercise(1,0)')
        
    with col2:
        slope = st.number_input('Slope of the peak exercise ST segment(1,0)')
        
    with col3:
        ca = st.number_input('Major vessels colored by flourosopy(1,0)')
